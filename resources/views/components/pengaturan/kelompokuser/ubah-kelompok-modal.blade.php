<x-modal.modal-update title="Ubah Kelompok User">
    <form method="POST"
        action="{{ route('kelompokuser.update', $role->id) }}?{{ http_build_query(request()->only(['search', 'sort_by', 'direction'])) }}"
        class="p-4 md:p-5">
        @csrf
        @method('PUT')
        <div class="grid gap-4 mb-4">
            <div class="col-span-2">
                ...
                <input type="text" name="nama" id="nama"
                    ...
                    placeholder="Masukkan Nama Kelompok" value="{{ $role->name }}" required>
            </div>
        </div>
