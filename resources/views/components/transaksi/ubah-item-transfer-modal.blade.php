<x-modal-update title="Ubah Transaksi Item Transfer">
    <form method="POST"
        action="{{ route('itemtransfer.update', $transaksi->id) }}?{{ http_build_query(request()->only(['search', 'gudang', 'start', 'end'])) }}"
        class="p-4 md:p-5">
        @csrf
        @method('PUT')
        <div x-data="ubahBarangSearch()" x-init="updateStok()" class="relative">
            ...
            <div class="mb-4"
                ...
                <label for="jumlah_stok_transfer"
                    class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Jumlah Stok Transfer</label>
