<x-modal.modal-update title="Ubah Transaksi Barang Masuk">
    <form method="POST"
        action="{{ route('barangmasuk.update', $transaksi->id) }}?{{ http_build_query(request()->only(['search', 'gudang', 'start', 'end', 'sort_by', 'direction'])) }}"
        class="p-4 md:p-5">
        @csrf
        @method('PUT')
        <div x-data="ubahBarangSearch()" x-init="updateStok()" class="relative">
            ...
            <div class="mb-4"
                ...
                <label for="jumlah_stok_masuk"
                    class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Jumlah Stok Masuk</label>
                <input type="number" name="jumlah_stok_masuk" id="jumlah_stok_masuk" min="1" step="1"
                    x-model="ubahJumlahStokMasuk" @input.debounce.500ms="updateStok()"
        ...
        <script>
            function ubahBarangSearch() {
                return {
                    ...
                    ubahJumlahStokMasuk: '{{ $editTransaksiSatuan['jumlah'] }}',
