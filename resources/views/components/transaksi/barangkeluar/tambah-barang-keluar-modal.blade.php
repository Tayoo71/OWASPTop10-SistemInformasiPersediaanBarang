<x-modal.modal-create title="Tambah Transaksi Barang Keluar">
    <form method="POST"
        action="{{ route('barangkeluar.store') }}?{{ http_build_query(request()->only(['search', 'gudang', 'start', 'end', 'sort_by', 'direction'])) }}"
        class="p-4 md:p-5">
        @csrf
