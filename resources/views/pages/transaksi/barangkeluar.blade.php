<x-layout>
            ...
            <div class="flex flex-grow">
                ...
                <div class="relative w-full flex">
                    <input type="search" id="search-dropdown" name="search"
                        ...
                        placeholder="Cari Transaksi Barang Keluar" value="{{ request('search') }}" />
                    <button type="submit"
    ...
    <div class="flex justify-between items-center mb-4">
        ...
        @endif
        @if ($transaksies->isNotEmpty() && $canExportBarangKeluar)
            ...
            <thead class="text-xs text-gray-700 bg-gray-50 sticky top-0 shadow-md">
                <tr>
                    <th scope="col" class="px-6 py-3 bg-gray-50">
                        <a href="{{ route('barangkeluar.index', array_merge(request()->query(), ['sort_by' => 'id', 'direction' => request('direction') === 'asc' ? 'desc' : 'asc'])) }}"
                    ...
                    <th scope="col" class="px-6 py-3 bg-gray-50">
                        <a href="{{ route('barangkeluar.index', array_merge(request()->query(), ['sort_by' => 'created_at', 'direction' => request('direction') === 'asc' ? 'desc' : 'asc'])) }}"
                            class="flex justify-center items-center">
                            TANGGAL BUAT
                    ...
                    <th scope="col" class="px-6 py-3 bg-gray-50">
                        <a href="{{ route('barangkeluar.index', array_merge(request()->query(), ['sort_by' => 'updated_at', 'direction' => request('direction') === 'asc' ? 'desc' : 'asc'])) }}"
                            class="flex justify-center items-center">
                            TANGGAL UBAH
                    ...
                    <th scope="col" class="px-6 py-3 bg-gray-50">
                        <a href="{{ route('barangkeluar.index', array_merge(request()->query(), ['sort_by' => 'kode_gudang', 'direction' => request('direction') === 'asc' ? 'desc' : 'asc'])) }}"
                            class="flex justify-center items-center">
                            GUDANG
                    ...
                    <th scope="col" class="px-6 py-3 bg-gray-50">
                        <a href="{{ route('barangkeluar.index', array_merge(request()->query(), ['sort_by' => 'nama_item', 'direction' => request('direction') === 'asc' ? 'desc' : 'asc'])) }}"
                            class="flex justify-center items-center">
                            NAMA BARANG
                    ...
                    <th scope="col" class="px-6 py-3 bg-gray-50">
                        <a href="{{ route('barangkeluar.index', array_merge(request()->query(), ['sort_by' => 'user_buat_id', 'direction' => request('direction') === 'asc' ? 'desc' : 'asc'])) }}"
                            class="flex justify-center items-center">
                            USER BUAT
                    ...
                    <th scope="col" class="px-6 py-3 bg-gray-50">
                        <a href="{{ route('barangkeluar.index', array_merge(request()->query(), ['sort_by' => 'user_update_id', 'direction' => request('direction') === 'asc' ? 'desc' : 'asc'])) }}"
                            class="flex justify-center items-center">
                            USER UPDATE
                            @if (request('sort_by') === 'user_update_id')
                                @if (request('direction') === 'asc')
                                    <svg class="w-4 h-4 ml-2" xmlns="http://www.w3.org/2000/svg" fill="none"
                                        viewBox="0 0 24 24" stroke="currentColor">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    ...
                    @if ($canUpdateBarangKeluar || $canDeleteBarangKeluar)
            ...
            <tbody>
                @forelse ($transaksies as $transaksi)
                    <tr class="odd:bg-white even:bg-gray-50 border-b">
                        ...
                        <td class="px-6 py-4 align-middle">{{ $transaksi['jumlah_stok_keluar'] }}</td>
                            ...
                            <td class="px-6 py-4 align-middle">
                                <div class="flex justify-center items-center">
                                    @if ($transaksi['statusBarang'] === true)
                                        @if ($canUpdateBarangKeluar)
                                            <a href="{{ route('barangkeluar.index', array_merge(request()->only(['search', 'gudang', 'start', 'end', 'sort_by', 'direction']), ['edit' => $transaksi['id']])) }}"
                                                class="font-medium text-yellow-300 hover:underline">
                                                Ubah
                                            </a>
                                        @endif
                                        @if ($canDeleteBarangKeluar)
                                            <a href="{{ route('barangkeluar.index', array_merge(request()->only(['search', 'gudang', 'start', 'end', 'sort_by', 'direction']), ['delete' => $transaksi['id']])) }}"
    ...
    @if ($canCreateBarangKeluar)
        <x-transaksi.barangkeluar.tambah-barang-keluar-modal :gudangs="$gudangs" />
    @endif
    @if ($transaksies->isNotEmpty() && $canExportBarangKeluar)
        {{-- Modal Export --}}
        <x-transaksi.barangkeluar.export-barang-keluar-modal />
    @endif
    @if ($editTransaksi && !$errors->any() && !session('error') && $canUpdateBarangKeluar)
        {{-- Modal Ubah Transaksi --}}
        <x-transaksi.barangkeluar.ubah-barang-keluar-modal :gudangs="$gudangs" :transaksi="$editTransaksi" :editTransaksiSatuan="$editTransaksiSatuan" />
    @elseif ($deleteTransaksi && !$errors->any() && !session('error') && $canDeleteBarangKeluar)
        {{-- Modal Hapus Transaksi --}}
        <x-modal.modal-delete :action="route(
            'barangkeluar.destroy',
            ['barangkeluar' => $deleteTransaksi->id] +
                request()->only('search', 'gudang', 'start', 'end', 'sort_by', 'direction'),
        )"
            message='Tindakan ini tidak dapat dibatalkan. Apakah Anda yakin ingin menghapus transaksi barang keluar dengan Nomor Transaksi "{{ $deleteTransaksi->id }}" | Nama Item "{{ $deleteTransaksi->barang->nama_item }}"?' />
    @endif
</x-layout>
